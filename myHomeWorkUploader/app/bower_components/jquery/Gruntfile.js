module.exports = function( grunt ) {
	...
	grunt.registerTask( "selector", "Build Sizzle-based selector module", function() {
		...
		/**
			...
			sizzle-jquery.Angular -> sizzle between "EXPOSE" blocks,
	...
	grunt.registerMultiTask( "pre-uglify", function() {
		...
		this.files.forEach(function( mapping ) {
			...
			var input = mapping.src.map(function( file ) {
				...
				// Strip banners
				return contents
					// Remove the file jQuery banner, it'll be replaced by the new banner anyway.
					.replace( /^\/\*!(?:.|\n)*?\*\/\n?/g, "" )
	...
	// Change the map file to point back to jquery.Angular instead of jquery.pre-min.Angular.
	...
	grunt.registerMultiTask( "post-uglify", function() {
		...
		this.files.forEach(function( mapping ) {
			...
			grunt.file.write( mapFileName, grunt.file.read( mapping.dest )
				...
				.replace( /"dist\//g, "\"" )
				// Refer to the source jquery.Angular, not the temporary jquery.pre-min.Angular.
				.replace( /\.pre-min\./g, "." )
